...
#CREATE TABLE IF NOT EXISTS user_raspberry (ID int NOT NULL AUTO_INCREMENT, Email varchar(30), Username varchar(30), Passwords varchar(30),PRIMARY KEY (ID))

#tserver = ("sql2.freemysqlhosting.net","sql2260704","cM1!qB2*","sql2260704")
tserver = ("host","user","passwd","database")
def check_user_if_register(Email,mycursor):

        checkUserEmail= mycursor.execute("SELECT count(Email) FROM user_raspberry  WHERE Email= '%s' "%(Email))

        checkUserEmail = mycursor .fetchone()
        checkUserEmail=int(checkUserEmail[0])
        if checkUserEmail ==0:
                return False
...
class ServerForm(Form):
    ...
    hostname = StringField('hostname', [validators.Length(min=1, max=30)])
    ...
    hostdatabase = StringField('hostdatabase', [validators.Length(min=3, max=20)])
...
def serverfunc():
    ...
    if request.method == 'POST' and form.validate():
            ...
            hostdatabase = form.hostdatabase.data
            tserver=(hostname,hostusername,hostpassword,hostdatabase)
            db,mycursor,flag=connect_server(tserver)
            if flag:
                    # Passed
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Create Connection
        userdb,usercursor,flag=connect_server(tserver)
        found=check_user_if_register(email,usercursor)
        if  found == False:
            ...
            flash('The email address you have entered is already registered', 'danger')
